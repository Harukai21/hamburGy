...
module.exports = {
  name: 'chat',
  description: 'Find a chatmate and start chatting with them',
  usage: '/chat',
  author: 'biru',
  ...
  execute(senderId, senderName, args, pageAccessToken, sendMessage) {
    ...
    // Notify both users
    sendMessage(senderId, { text: `🎉 You have been paired with ${chatmateId}! Say hi! Type "/quit" to leave the chat.` }, pageAccessToken);
    sendMessage(chatmateId, { text: `🎉 You have been paired with ${senderId}! Say hi! Type "/quit" to leave the chat.` }, pageAccessToken);
  },

  // Handle when a user wants to quit the chat
  quit(senderId, pageAccessToken, sendMessage) {
    if (!activeChats.has(senderId)) {
    ...
    sendMessage(senderId, { text: 'You have left the chat.' }, pageAccessToken);
    sendMessage(chatmateId, { text: 'Your chatmate has left the chat.' }, pageAccessToken);
  ...
  routeMessage(senderId, message, pageAccessToken, sendMessage) {
    if (!activeChats.has(senderId)) {
      return sendMessage(senderId, { text: 'You are not in an active chat!' }, pageAccessToken);
    }

    const chatmateId = activeChats.get(senderId);
    sendMessage(chatmateId, { text: message }, pageAccessToken);
  }
};
