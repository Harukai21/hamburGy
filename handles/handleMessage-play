...
async function handleMessage(event, pageAccessToken) {
  ...
  if (messageText) {
    ...
    if (messageText.startsWith(prefix)) {
      ...
      // Handle manual video selection (e.g., /1, /2, /3)
      if (['1', '2', '3'].includes(commandName)) {
        const playCommand = commands.get('play');
        if (playCommand) {
          try {
            await playCommand.handleSelection(senderId, parseInt(commandName) - 1, pageAccessToken, sendMessage);
          } catch (error) {
            console.error(`Error processing selection ${commandName}:`, error);
            sendMessage(senderId, { text: 'There was an error processing your selection.' }, pageAccessToken);
          }
        }
        return;
      }
